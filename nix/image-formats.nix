...
let
  ...
  image-with-secrets-builder = { pkgs, image-path, start-vm ? "" }:
    pkgs.callPackage ({ }:
  ...
  imageFormats = {
    qcow = { config, pkgs, ... }: {
      imports = [ inputs.nixos-generators.nixosModules.qcow ];
      # system.build.thymis-image = config.system.build.qcow;
      system.build.thymis-image-with-secrets-builder =
    ...
    sd-card-image = { config, pkgs, modulesPath, extendModules, ... }:
      let
        rootfsImage = hostPkgs:
          hostPkgs.callPackage "${modulesPath}/../lib/make-ext4-fs.nix" ({
              ...
              modules = [
                ({ config, ... }: {
                  users.users.root.openssh.authorizedKeys.keys =
                    lib.mkForce [ ];
                  system.build.sdImage = lib.mkForce (sdImage config
              ...
              modules = [
                ({ config, ... }: {
                  ...
                  system.build.sdImage = lib.mkForce
                    (sdImage config inputs.nixpkgs.legacyPackages.x86_64-linux);
                })
              ];
            }).config.system.build.sdImage;
          };
    ...
    usb-stick-installer =
          ...
          modules = [
            {
              ...
              isoImage.isoName =
                "${variant.config.isoImage.isoBaseName}-${config.system.nixos.label}-${pkgs.stdenv.hostPlatform.system}.iso";
      ...
      in {
        imports = [ inputs.thymis.inputs.disko.nixosModules.disko ];
        disko.devices = {
          disk = {
            main = {
